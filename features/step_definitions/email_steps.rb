...
When /^(?:I|they|participant|reseller|client manager) follow(?:s?) "([^"]*?)" link in inbox of "([^"]*?)" with subject as "([^"]*?)"/ do |link,email_address,subject|
  open_email(email_address, :with_subject => subject)
  visit_in_email(link)
end
